...
> `smoke:ui:full` 一级同时执行 **61 项**界面断言（断言内容见 [docs/TEST_CASES.md](docs/TEST_CASES.md) 的 A7 / A8 / B0 节，
...
npm run smoke:ui:file      # 上一项 + 加载真实视频后再截图（49 项检查）
npm run smoke:ui:full      # 再额外在应用内真的点一次「开始转换」并等任务跑完（61 项检查）
```

**当前实测结果**：`npm run smoke` **56/56 通过**（0 失败，总耗时 18.4s），
`npm run smoke:ui:full` **61/61 通过、退出码 0**（三级命令的检查项是递进追加的：14 → 49 → 61），
...
| **没有单元测试框架** | 没有 Vitest / Jest，纯函数（`renderer/utils/format.ts`、`progress.ts` 的解析、`sanitizeFileName()`）未做边界穷举。当前只有端到端冒烟（56 项）与界面自检（三级 14 / 49 / 61 项）两层 |
