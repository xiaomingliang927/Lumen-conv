...
> 内容语义完全一致（状态「已完成」、产物 706 KB、可展开命令与日志），所以这里只标注约数。
...
> `smoke:ui:full` 一级同时执行 **48 项**界面断言（断言内容见 [docs/TEST_CASES.md](docs/TEST_CASES.md) 的 A7 / A8 / B0 节，
...
| 怕选错 | 默认只露"输出格式 / 质量 / 分辨率"三层，字幕、音轨、裁剪、文件名模板折叠进「高级选项」 |
...
│             ├── DetailsPanel.vue  视频信息 + 输出格式 + 质量尺寸 + 高级选项 │
...
npm run smoke:ui:file      # 上一项 + 加载真实视频后再截图（36 项检查）
npm run smoke:ui:full      # 再额外在应用内真的点一次「开始转换」并等任务跑完（48 项检查）
```

**当前实测结果**：`npm run smoke` **56/56 通过**（0 失败，总耗时 18.4s），
`npm run smoke:ui:full` **48/48 通过、退出码 0**（三级命令的检查项是递进追加的：14 → 36 → 48），
...
1. 在左侧点选一个文件，右侧「输出格式」区按 **常用 / 压缩 / 音频 / 进阶** 四组列出 9 个预设，每个都带一句说明；
2. 不确定就选「MP4 通用兼容」——兼容性最好，遇到"设备不支持"优先试它；
3. 选完预设后可以继续微调：**视频编码器**（显卡加速项会按实际可用性置灰并说明原因）、**质量档位**（存档级 → 极小体积）、
   **分辨率**、**帧率**；
4. 展开「高级选项」还能设置：音轨选择、字幕轨道、片段裁剪（起止秒）、文件名模板（支持 `{name}` `{preset}` `{date}`）、
   是否保留元数据；
...
| **没有单元测试框架** | 没有 Vitest / Jest，纯函数（`renderer/utils/format.ts`、`progress.ts` 的解析、`sanitizeFileName()`）未做边界穷举。当前只有端到端冒烟（56 项）与界面自检（三级 14 / 36 / 48 项）两层 |
