...
> `smoke:ui:full` 一级同时执行 **81 项**界面断言（断言内容见 [docs/TEST_CASES.md](docs/TEST_CASES.md) 的 A7 / A8 / B0 节，
...
npm run smoke:ui:full      # 再额外在应用内真的点一次「开始转换」并等任务跑完（81 项检查）
```

**当前实测结果**：`npm run smoke` **77/77 通过**（0 失败，总耗时 18.4s），
`npm run smoke:ui:full` **81/81 通过、退出码 0**（三级命令的检查项是递进追加的：14 → 62 → 74），
