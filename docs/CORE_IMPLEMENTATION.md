...
当前共 **60 项通过 / 0 失败**（实测数据见 `docs/TEST_CASES.md` 的 A 部分）。
...
| `npm run smoke:ui:file` | 加 `--smoke-file=…`，通过 `window.__lumenAddFiles` 走真实 `addFiles` 路径加载一个视频 | **62 项** |
| `npm run smoke:ui:full` | 再加 `--smoke-convert`，在应用内真的点一次「开始转换」并等任务跑到终态 | **74 项** |
...
实测 **74/74 通过、退出码 0**（含应用内真实转换：状态 `done`、产物 3.15 MB、进度 100%）。
