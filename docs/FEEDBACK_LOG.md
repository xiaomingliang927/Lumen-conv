...
| 界面走查阶段（本轮第二轮） | 项目所有者（**真实用户反馈**） | 在成品上实际操作后，针对**自定义模式**提出：**"自定义不要这个推荐，缺少专业性，这个用最初那个版本自己调整更合适"**（配图是「你要拿去干什么」的 8 张用途卡片与"选一个用途就够了，细节我来定"） | 上一轮（D-018）只解决了"两种模式看不出区别"，这一轮是把**自定义模式的定位**改对：它不该有推荐。落地五项改动：① 用途卡片整块在自定义模式下不渲染（连同"已偏离推荐值"提示与用途小贴士）；② 专业参数**常开**，删掉折叠开关与 `advancedOpen` 状态（回到"最初那个版本"直接调参数的形态）；③ 摘要行在自定义模式下显示「自定义参数」；④ **兼容性预检拆成独立区块、两种模式都显示**（它拦的是事实性错误，不能跟着推荐一起消失）；⑤ 区块顺序改由 CSS `order` 控制。<br>新增两条验收断言：「自定义模式：不出现用途推荐卡片」实测 **0 张**、「推荐模式：用途推荐卡片齐全」实测 **8 张**。<br>证据：`smoke:ui:file` **68/68**、`smoke:ui:full` **83/83**、便携版 **83/83**，退出码均 0；对照截图 `mode-recommended.png` / `mode-custom.png`。<br>完整决策与三项代价（自定义模式首屏不再有视频信息 / 偏离提示在该模式下不可见 / 页面更长）见 `DECISIONS.md` D-019 |
...
| Electron 运行时要不要现在修好？（上轮第 9 项） | **已修好**。`npm run smoke:ui:full` 能启动真实窗口、加载真实视频、在应用内真的跑完一次转换、截图，并以退出码 0 结束（**83/83 通过**） |
...
| 界面自检要不要覆盖"点按钮真的能开始转换"？（由开发者自测提出，非用户反馈） | **已覆盖且断言升级为严格判定**。`npm run smoke:ui:full` 现在是 **83/83 通过、退出码 0**：点击前先 `engine.remove()` 清空队列，点击后要求引擎队列长度从 **0 变为 1**。正是这条断言暴露了"按钮静默失效"的克隆缺陷（见 `TEST_CASES.md` 的 A7 与 `SESSION_SUMMARY.md` 第 5.1.1 节） |
...
2. **是否要在有网络的环境补一个 NSIS 安装包？**
   便携版已经有了并实测跑通（`release/Lumen-conv-便携版/`，602.4 MB，83/83 自检通过），
...
| 便携版打包（上一轮新增能力，**非用户反馈**） | ✅ 已落地：`scripts/package-portable.mjs` 离线组装出 `release/Lumen-conv-便携版/`（`Lumen-conv.exe` 200.4 MB、目录 602.3 MB），实跑自检 83/83 通过；**NSIS 安装包仍未产出**，原因是环境限制而非设计取舍（见 `SESSION_SUMMARY.md` 第 4.3 节） |
