...
| Electron 运行时要不要现在修好？（上轮第 9 项） | **已修好**。`npm run smoke:ui:full` 能启动真实窗口、加载真实视频、在应用内真的跑完一次转换、截图，并以退出码 0 结束（**74/74 通过**） |
...
| 界面自检要不要覆盖"点按钮真的能开始转换"？（由开发者自测提出，非用户反馈） | **已覆盖且断言升级为严格判定**。`npm run smoke:ui:full` 现在是 **74/74 通过、退出码 0**：点击前先 `engine.remove()` 清空队列，点击后要求引擎队列长度从 **0 变为 1**。正是这条断言暴露了"按钮静默失效"的克隆缺陷（见 `TEST_CASES.md` 的 A7 与 `SESSION_SUMMARY.md` 第 5.1.1 节） |
...
2. **是否要在有网络的环境补一个 NSIS 安装包？**
   便携版已经有了并实测跑通（`release/Lumen-conv-便携版/`，602.4 MB，74/74 自检通过），
...
6. **冒烟测试没覆盖的样本要不要补？**
   `npm run smoke` 已覆盖 60 项并全部通过（含批量转换 2 项），但以下几类仍无样本：
...
| 反馈 | 状态 |
| --- | --- |
| 7 条原始要求 | 6 条已落地（① ② ③ ④ ⑥ ⑦ 有代码依据，其中 ① ② ③ 已由 `npm run smoke` 的 60 项检查实机验证，⑦ 另有界面自检的 74 项断言，含"按钮点击真的创建了任务"与"质量档位真的影响 ffmpeg 命令"），第 ⑤ 条（提交 github）**已执行**（见下表） |
| 选定 Electron 技术栈 | ✅ 已落地（后端逻辑与界面都已实机验证：`smoke` 60/60、`smoke:ui:full` 74/74、退出码 0） |
...
| 打包阶段自测发现的两个问题（**非用户反馈**，本轮） | ✅ 已修复：① 打包态 `app.getAppPath()` 指向文件导致的 `ENOTDIR` → 新增 `smokeBaseDir()`，便携版自检 **74/74 通过、退出码 0**；② `queue.png` 被 `queue-done.png` 覆盖 → 跑过转换就不再写 `queue.png`，两者现已哈希不同；③ 顺带把队列卡片那条断言从"只打印"升级为真断言。见 `SESSION_SUMMARY.md` 第 5.1 节第 9-11 条、`TEST_CASES.md` 的 A8、`DECISIONS.md` 的 D-017 |
| 便携版打包（上一轮新增能力，**非用户反馈**） | ✅ 已落地：`scripts/package-portable.mjs` 离线组装出 `release/Lumen-conv-便携版/`（`Lumen-conv.exe` 200.4 MB、目录 602.3 MB），实跑自检 74/74 通过；**NSIS 安装包仍未产出**，原因是环境限制而非设计取舍（见 `SESSION_SUMMARY.md` 第 4.3 节） |
| "推荐设置和自定义没区别啊"（本轮，**真实用户反馈**） | ✅ 已修复并**量化验证**：先测出两种模式改动前的首屏区块逐块相同，再改四处（视频信息默认收起 / 新增方案摘要 / 专业参数入口搬到首屏最上面 / 取消切模式自动展开）。新增断言"自定义模式：专业参数入口出现在首屏"实测 `专业参数@152`；`smoke:ui:file` **62/62**、`smoke:ui:full` **74/74**，退出码 0。截图 `docs/screenshots/mode-recommended.png` 与 `mode-custom.png` 是同一时刻两种模式的对照（自定义模式顶部多出「专业参数」入口，其余骨架一致）。决策与代价见 `DECISIONS.md` D-018 |
| "自定义不要这个推荐，缺少专业性"（本轮第二轮，**真实用户反馈**） | ✅ 已落地并把定位改对：自定义模式=**纯参数面板**——用途推荐卡片整块不渲染（实测 0 张）、专业参数常开不再折叠、兼容性预检保留且两种模式都显示。新增 2 条专门断言（自定义 0 张卡片 / 推荐 8 张卡片），`smoke:ui:file` **62/62**、`smoke:ui:full` **74/74**、便携版 **74/74**，退出码均 0。决策与三项代价见 `DECISIONS.md` D-019 |
| "用途与在哪播耦合" + "换成手机屏幕比例没变"（本轮第三轮，**真实用户反馈**） | ✅ 两条都已修复，且都找到了比反馈更深的根因：<br>① **用途带出设备**（8 个用途各自映射，选「老电视/车载 U 盘」→ 设备自动变 `android-old`，那个用途的兼容性预检这才真正跑起来）；设备提示写明"由用途自动设定"、手动改过后给「用回用途推荐」；**体积上限**抽成 `SizeLimitField.vue` 归还用途区（推荐模式）/ 质量区（自定义模式），一个参数只有一个主人。<br>② 新增 `shared/output-size.ts` 作为**输出尺寸与 ffmpeg 滤镜的唯一计算处**，界面直接显示「输出尺寸 640×360（源低于 1080p 上限，不放大）」；新增默认关闭的「画面比例」竖屏 9:16 适配（画布短边取 min(上限, 源短边)，竖屏下同样不放大）。<br>证据：`smoke` **60/60**、`test:real:convert` **26/26**（含 3 条 ffprobe 回读真实产物尺寸：4K→720×1280、360p 源→360×640）、`smoke:ui:file` **62/62**、`smoke:ui:full` **74/74**、便携版 **74/74**，退出码均 0 |
