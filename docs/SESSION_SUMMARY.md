...
| 项 | 实际值 |
| --- | --- |
| 交付形态 | Windows 桌面客户端（Electron 38 + Vue 3 + Vite + TypeScript）。便携版目录 **560.2 MB**、精简版目录 **282.2 MB**；GitHub Release 三个附件：**153.6 MB（.7z）/ 237.1 MB（.zip）/ 119.5 MB（精简版）** |
| 7 条原始需求 | 全部落地；逐条对照见第 3 节与 `README.md` 的"需求对照" |
| 工作轮次 | 首轮实现 + **4 轮真实用户反馈迭代** + 1 轮定向优化（用户从 10 个方向里勾了 6 项）+ 1 轮分发发布 |
| 决策记录 | **26 条** ADR 风格决策（`docs/DECISIONS.md` D-001 … D-026），每条含备选方案、代价与适用边界 |
| 反馈记录 | **5 轮**（`docs/FEEDBACK_LOG.md`），均含用户原话、响应动作与验证证据 |
| 自动化检查 | `smoke` **77** + `test:real:convert` **28** + 界面自检 **68 / 83 / 16** + 打包态 **82**（全部有退出码，可重复执行） |
...
> （本轮从最初的 14 项基础检查一路加到 `smoke:ui:full` **83 项**），历史条目里的数字也一并按最新值刷新过，
...
- **GitHub Release**：`v1.0.0`，附件 `Lumen-conv-portable-v1.0.0.zip`（237.1 MB）
  - 上传后 GitHub 把中文附件名清洗成了 `Lumen-conv-.-v1.0.0.zip`，
    看着像坏文件，已用 API `PATCH` 改名成纯 ASCII（秒级，不必重传 237 MB）
  - 已验证**匿名可下载**：HTTP 200、237.1 MB、前 4 字节是合法 ZIP 魔数
...
1. **换压缩算法**：用 7z（LZMA2）重压，**237.1 MB → 153.6 MB**（省 35%）。
...
2. **打包脚本自动瘦身**：Electron 自带 55 个语言包共 **43.7 MB**，本应用是全中文界面，
   ...
   便携版目录 602.4 MB → **560.2 MB**，并**实测自检 82/82 通过**（删语言包有启动失败风险，必须先验证）。
...
| `Lumen-conv-portable-v1.0.0.zip` | 237.1 MB | 通用格式 |
...
- `docs/DECISIONS.md`：**26 条** ADR 风格决策记录（D-001 … D-026）。每条都写清"背景 / 决策 / 代价权衡 /
...
npm run smoke:ui:cli   # 命令行直接带一个视频路径启动，验证 Shell 集成入口（16 项检查）
...
| **桌面应用已真正启动** | `npm run smoke:ui:full` 启动真实 Electron 窗口、加载真实视频、逐页截图、**在应用内真的点一次「开始转换」并等任务跑到终态**，**83/83 通过、退出码 0**，产出 9 张 PNG（见 4.1） |
| **便携版打包已产出并实机验证** | `npm run dist:portable`（`node scripts/package-portable.mjs --build`）**完全离线**产出 `release/Lumen-conv-便携版/`：`Lumen-conv.exe` **200.4 MB**（210,149,888 字节）、目录总计 **602.4 MB**；`release/` 已被 `.gitignore` 排除（`git check-ignore -v release/` 命中第 4 行）。产物**实跑全套界面自检 82/82 通过、退出码 0**（含应用内真实转换：状态 `done`、产物 3.15 MB、进度 100%），详见 4.1.1 |

#### 4.1.1 便携版（打包态）自检：`release/Lumen-conv-便携版/Lumen-conv.exe` **82/82 通过、退出码 0**
...
| 自检结果 | **83/83 通过、退出码 0**，其中包含应用内真实转换：任务状态 `done`、产物 **3.15 MB**、进度 `percent=100` |
...
2. 第二次（本轮核对覆盖范围时才发现）：加载第二个样本的路径也用了它，于是
   **8 项多文件检查在打包态一次都没跑过**，摘要却照样打印"58/58 通过"——
   而文档一直宣称打包态与开发态"完全相同"。修复后打包态也是 **82/82**（见 5.1 第 14 条）。
...
#### 界面自检：三级命令，最高一级 `npm run smoke:ui:full` **83/83 全部通过**
...
三个级别是递进关系，检查项只在上一级基础上**追加**（14 → 68 → 83）：

| 命令 | 追加的动作 | 检查项 |
| --- | --- | --- |
| `npm run smoke:ui` | 启动窗口 → 等 `data-store-ready` → 逐页切换截图 | **14 项** |
| `npm run smoke:ui:file` | 加 `--smoke-file=…`，加载真实视频后再断言信息面板与缩略图 | **68 项** |
| `npm run smoke:ui:full` | 加 `--smoke-convert`，在应用内真的点一次「开始转换」并等任务跑到终态 | **83 项** |

`smoke:ui:file` 一级（62 项）的断言与实测结果：
...
**结果：83/83 通过，退出码 0，且控制台不再出现任何 `An object could not be cloned.` 错误**
...
> 本轮状态变化：
> ① 原先列在本表的 **`--smoke-convert`（应用内真跑转换）已移出**——`npm run smoke:ui:full` 实测 **83/83 通过、退出码 0**（含"按钮点击真的创建了任务"这条严格断言），证据见 4.1 与 `docs/screenshots/queue-done.png`。
...
产物 `release/Lumen-conv-便携版/` 已实机验证 **82/82 通过、退出码 0**（见 4.1.1）。
...
| Release 附件 | `Lumen-conv-portable-v1.0.0.zip`（**237.1 MB**，完整便携版 602.4 MB 压缩后）。已用匿名请求验证：HTTP 200、大小一致、前 4 字节是合法 ZIP 魔数 |
...
| 23 | **便携版 602 MB 太重**（第 6 项）：为了试一下要下 600 MB，其中 278 MB 是两个 ffmpeg 二进制 | 新增 `npm run dist:portable:slim`：不内置 ffmpeg，目录 **602.4 MB → 324.4 MB**，输出到**另一个目录**（不覆盖完整版，否则会让人以为瘦身没生效）。前提是应用本来就有"找不到 ffmpeg 的引导"，所以不是砍功能，而是把"自带"换成"你自己那份也能用"<br>**精简版逼出一个真 bug（本次最有价值的收获）**：`capabilities:get` 在找不到 ffmpeg 时是**抛错**的，于是渲染进程拿不到 capabilities，而引导横幅的显示条件 `capabilities !== null && !capabilities.ready` **永远不成立** —— 用户打开精简版转换全不可用，界面却没有任何解释，标题栏永远停在"检测中…"。这正踩在需求 7（"给人用"）最不能接受的形态上：**功能不可用且不说原因**。改为返回"未就绪"的能力对象 + 三条人话诊断 | `smoke:ui:file` 68/68、`smoke:ui:full` 82/82（开发态）、完整便携版 **82/82**、精简版自检 **14/14**。修复后精简版标题栏显示「ffmpeg 未就绪」、顶部横幅写明两步操作并给「去设置」按钮（截图见 `release/Lumen-conv-精简版/docs/screenshots/main.png`）。**打包态比开发态少 1 项是有意的**（那条注册表安全断言只在开发态成立），已写进 `DECISIONS.md` D-025 而不是含糊说"完全相同" |
| 24 | **预览太小看不清**（用户看到成品后的反馈）：第一版把预览放在右侧 400px 面板里，两张图各约 190px |  搬到**中间栏下半部分**（加载少量文件时那里本来是一大片空白）：两张图各约 390px，约 4 倍面积；新增**点击放大到整屏**（原图/效果可切换，Esc 关闭）；渲染宽度 480 → 960；文件列表改成不参与拉伸（`flex: 0 0 auto` + 最高 46%）。<br>**这条的教训**：从断言角度看第一版完全合格（两张图都在、参数一变就重渲染），但 190px 的格子里用户根本看不出差别 —— **断言测不出"够不够大"**，这类问题只能靠人看 |  自检同步更新选择器（`.preview-pane .preview-grid`）后 `smoke:ui:file` 68/68；截图 `main-with-file.png` 现在能直接看到并排的大图 |
...
> 而不是从旧源码里读出来的。修复后的代码、实验输出与 `smoke:ui:full` 的 83/83 都是可直接核对的。
