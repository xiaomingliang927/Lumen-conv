...
npm run smoke:ui:file  # 上一项 + 通过 __lumenAddFiles 加载真实视频后再截图（36 项检查）
npm run smoke:ui:full  # 再额外在应用内真的点一次「开始转换」并等任务跑完（48 项检查）
```
...
| **桌面应用已真正启动** | `npm run smoke:ui:full` 启动真实 Electron 窗口、加载真实视频、逐页截图、**在应用内真的点一次「开始转换」并等任务跑到终态**，**48/48 通过、退出码 0**，产出 5 张 PNG（见 4.1） |
| **便携版打包已产出并实机验证** | `npm run dist:portable`（`node scripts/package-portable.mjs --build`）**完全离线**产出 `release/Lumen-conv-便携版/`：`Lumen-conv.exe` **200.4 MB**（210,149,888 字节）、目录总计 **602.3 MB**；`release/` 已被 `.gitignore` 排除（`git check-ignore -v release/` 命中第 4 行）。产物**实跑全套界面自检 48/48 通过、退出码 0**（含应用内真实转换：状态 `done`、产物 706 KB、进度 100%），详见 4.1.1 |

#### 4.1.1 便携版（打包态）自检：`release/Lumen-conv-便携版/Lumen-conv.exe` **48/48 通过、退出码 0**
...
| 自检结果 | **48/48 通过、退出码 0**，其中包含应用内真实转换：任务状态 `done`、产物 **706 KB**、进度 `percent=100` |
...
这层验证的价值在**本轮被真实证明**：开发态 48/48 全绿，并不意味着打包态也能跑——`app.getAppPath()` 在打包态
...
#### 界面自检：三级命令，最高一级 `npm run smoke:ui:full` **48/48 全部通过**
...
三个级别是递进关系，检查项只在上一级基础上**追加**（14 → 36 → 48）：

| 命令 | 追加的动作 | 检查项 |
| --- | --- | --- |
| `npm run smoke:ui` | 启动窗口 → 等 `data-store-ready` → 逐页切换截图 | **14 项** |
| `npm run smoke:ui:file` | 加 `--smoke-file=…`，加载真实视频后再断言信息面板与缩略图 | **36 项** |
| `npm run smoke:ui:full` | 加 `--smoke-convert`，在应用内真的点一次「开始转换」并等任务跑到终态 | **48 项** |

`smoke:ui:file` 一级（36 项）的断言与实测结果：
...
**结果：48/48 通过，退出码 0，且控制台不再出现任何 `An object could not be cloned.` 错误**
...
> 本轮状态变化：
> ① 原先列在本表的 **`--smoke-convert`（应用内真跑转换）已移出**——`npm run smoke:ui:full` 实测 **48/48 通过、退出码 0**（含"按钮点击真的创建了任务"这条严格断言），证据见 4.1 与 `docs/screenshots/queue-done.png`。
...
产物 `release/Lumen-conv-便携版/` 已实机验证 **48/48 通过、退出码 0**（见 4.1.1）。
...
> 而不是从旧源码里读出来的。修复后的代码、实验输出与 `smoke:ui:full` 的 48/48 都是可直接核对的。
