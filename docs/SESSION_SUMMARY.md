...
| 决策记录 | **28 条** ADR 风格决策（`docs/DECISIONS.md` D-001 … D-028），每条含备选方案、代价与适用边界 |
| 反馈记录 | **5 轮**（`docs/FEEDBACK_LOG.md`），均含用户原话、响应动作与验证证据 |
| 自动化检查 | `smoke` **77** + `test:real:convert` **28** + 界面自检 **69 / 84 / 17** + 打包态 **82**（全部有退出码，可重复执行） |
| 界面截图 | **10 张**，全部由自检命令自动生成，不是手工摆拍 |
| 代码托管 | 已推送 GitHub（`main`），并在 **`v1.0.0`** 发布 Release（含便携版附件，已验证匿名可下载） |
| 已知未验证 | 见 4.2 节，逐条注明"为什么没验证"，不用"应该没问题"糊过去 |

> 这份文档的读法：第 2 节讲**怎么演进的**（含每次用户反馈改变了什么），
> 第 3 节讲**交付了什么**，第 4 节讲**验证到什么程度、什么没验证**，
> 第 5 节讲**发现并修掉了什么缺陷**（含两次"假通过"复盘），第 6 节讲**还差什么**。
> 决策的备选方案与代价在 `docs/DECISIONS.md`，用户反馈原话在 `docs/FEEDBACK_LOG.md`。

---

> **计数口径**：本文档里的检查项数都是**最新一次实测**的值。自检项会随功能增加而增长
> （本轮从最初的 14 项基础检查一路加到 `smoke:ui:full` **84 项**），历史条目里的数字也一并按最新值刷新过，
> 所以它表示的是"这条修复**目前**由多少个检查覆盖"，**不等于**"当时就有这么多项"。
> 需要知道某一轮的原始数字时，看 git 历史里那份文档。

开发方式为**人机协作**：人类（项目所有者）负责给出需求与约束、做关键选型拍板、提供仓库凭据、
放宽环境限制，并在**每一轮成品上实际操作后给出反馈**；AI 负责环境探测、方案设计、全部代码编写、
构建联调与文档产出。第 2 节按阶段归纳，不逐条罗列操作流水；
**每次用户反馈的原话、根因与响应**在第 2 节阶段七与 `docs/FEEDBACK_LOG.md` 里逐条对应。
...
### 阶段七：真实用户反馈迭代（4 轮，改了 4 处界面结构）

这一阶段的驱动力**全部来自用户实际操作后的反馈**，而不是自测。四轮的原话与响应：

| 轮次 | 用户原话 | 根因（量化后） | 改动 |
| --- | --- | --- | --- |
| 1 | "推荐设置和自定义没区别啊" | 让自检打印两种模式的**首屏可见区块**后实测：两边逐块相同（`视频信息@106 → 用途@376 → 在哪播@787`），专业参数在 900px 之外 | 视频信息默认收起成一行摘要；新增「当前方案摘要」；专业参数入口搬到面板最上面；取消"切模式自动展开" |
| 2 | "自定义不要这个推荐，缺少专业性，这个用最初那个版本自己调整更合适" | 第 1 轮只做到"让差别可见"，没回答"自定义模式该是什么" | 自定义模式下**用途卡片整块不渲染**；专业参数**常开**（删掉折叠开关）；摘要显示「自定义参数」；**兼容性预检拆成独立区块、两种模式都显示**（它是事实不是推荐） |
| 3 | "你要拿去干什么和在哪播放/多大体积存在耦合情况" | 查证发现比"耦合"更糟：**语义重叠但没接上** —— 体积上限控件住在「在哪播」里、值由用途决定（一个参数两个主人）；用途有「老电视/车载U盘」、设备下拉有「老安卓电视/车机」，但选前者**根本不会改后者**，兼容性预检按"不限定"跑 | `UseCase` 新增 `deviceId`（8 个用途各自映射），两条写入路径都补上；设备提示写明来源、手动改过后给「用回用途推荐」；体积上限抽成组件归还用途区/质量区 |
| 4 | "我换成手机的但是屏幕比例没变" | 分辨率是**上限**，实现有"只缩不放"规则：640×360 的源选 1080p，产物仍是 640×360，而界面写着「1080p 全高清」、卡片写着"按手机屏幕尺寸" —— **界面显示了一个它不会产出的分辨率** | 抽出 `shared/output-size.ts` 作为**输出尺寸与 ffmpeg 滤镜的唯一计算处**；界面直接显示「输出尺寸 640×360（源低于 1080p 上限，不放大）」；新增默认关闭的「画面比例」竖屏 9:16 适配 |

**这一阶段最值得记的两件事**

1. **"功能存在 ≠ 用户感知到"**。第 1 轮的代码在逻辑上完全正确（两种模式确实不同），
   但差异全在折叠线以下 —— 用户说"没区别"是准确的观察。修法的关键不是加功能，
   而是**先量化"用户第一眼看到什么"**（打印首屏可见区块），再据此改结构。
2. **断言测不出"够不够大"**。第 5 项优化（单帧预览）第一版把两张图放在右侧 400px 面板里
   （各约 190px），断言全过 —— 两张图都在、参数一变就重渲染。用户看了一眼说
   "太小了看不清能不能在中间找个大点的位置显示"。搬到中间栏后各约 390px，
   并加了点击放大到整屏。**这类问题只能靠人看**，自动化检查覆盖不到。

### 阶段八：定向优化（用户从 10 个方向里勾选 6 项）

用户问"还想进行优化帮我给出几个方向"。给出的方向基于**先核对代码确认真实缺口**
（确认当时无 CI、无单测框架、无托盘、无命令行接文件、无音频处理、无拼接、无暂停/重排、无自定义预设），
并明确列出 5 条"不建议现在做"及理由（NSIS 安装包卡在受限网络；自动更新需要签名与服务器；
代码签名需要证书；多语言收益低；HDR 与 NVENC 实测缺样本与硬件）。

用户勾选的 6 项与落地结果：

| # | 方向 | 落地内容 | 决策 |
| --- | --- | --- | --- |
| 1 | 音频处理 | 响度归一化（EBU R128，-16 LUFS）、音量增益 ±30 dB、声道转换。**顺序硬规则：先增益、后归一化**（反过来的话 loudnorm 会把增益拉平） | D-022 |
| 2 | 字幕烧录 | hardcode 烧进画面。路径转义与 `si` 换算集中在 `shared/subtitle-burn.ts`；两道 block 级拦截（直通不能烧录、图形字幕不能烧录），**改参数的当下就报警** | D-022 |
| 3 | 队列能力 | 引擎级暂停/继续（暂停 = **不再启动新任务，跑着的跑完** —— ffmpeg 不支持断点续传，硬停只能从头再来，那叫取消）、任务重排、队列页调并发、总剩余时间、**开转前磁盘空间预检**（只警告不硬拦） | D-023 |
| 4 | Shell 集成 | 命令行接文件（拖到 exe / 发送到 / 右键菜单三条入口的共同底座）、已开着时由 `second-instance` 转交、只写 HKCU 可撤销、**不动文件关联**、**开发态拒绝写注册表** | D-024 |
| 5 | 单帧实时预览 | 与正式转换**复用同一份**尺寸/字幕规则，改参数防抖 400ms 重渲染；文案明确"能看画面、不能看编码质量" | D-025 |
| 6 | 便携版瘦身 | 精简版 602.4 MB → **324.4 MB**（不内置 ffmpeg），输出到独立目录不覆盖完整版；**其后又压到 282.2 MB**（语言包瘦身） | D-025 / D-026 |

**这一阶段最有价值的收获**：精简版**逼出了一个真 bug** —— `capabilities:get` 在找不到 ffmpeg 时
是抛错的，于是渲染进程拿不到 `capabilities`，而引导横幅的显示条件
（`capabilities !== null && !capabilities.ready`）**永远不成立**：用户打开精简版，
转换全点不动，界面上却没有任何解释，标题栏永远停在"检测中…"。
这正踩在需求 7（"给人用"）最不能接受的形态上 —— **功能不可用且不说原因**。
换一种运行前提（没有内置 ffmpeg），就逼出了只在那种前提下才暴露的缺陷。
...
| 53.5 MB | 22.6% | `ffmpeg.exe` |
| 53.4 MB | 22.5% | `ffprobe.exe` |
| ~43 MB | — | 其余 dll / pak / 语言包 |

两个 ffmpeg 二进制合计 **107 MB**，是框架之外的最大项。据此做了三件事：

1. **换压缩算法**：用 7z（LZMA2）重压，**227.4 MB → 153.6 MB**（省 35%）。
   同一份内容，只是压缩更强；Windows 11 资源管理器原生支持解压 `.7z`（本机 26200 已验证前提成立）。
2. **打包脚本自动瘦身**：Electron 自带 55 个语言包共 **43.7 MB**，本应用是全中文界面，
   只保留 zh-CN / en-US / en-GB（省 42.2 MB）；同时排除自检写在便携版目录里的截图（测试产物不该进分发包）。
   便携版目录 602.4 MB → **560.2 MB**，并**实测自检 84/84 通过**（删语言包有启动失败风险，必须先验证）。
3. **补一个精简版附件**：不含 ffmpeg，**119.5 MB**，适合本机已有 ffmpeg 的人。

最终 Release 三个附件（均已验证匿名可下载 + 文件头字节正确）：
...
两个镜像的**完整加速地址**已写进 README 与 Release 说明（把原始地址接在镜像后面即可）。
两点如实标注：① 速度是**本机实测**，不同网络/时段差别很大，以用户本地为准；
② 第三方镜像会看到下载流量，介意的话就用最小的 `.7z` 直连。

> 结论：**直连基本不可用（142 分钟）**，压小文件（237→153.6 MB）与换镜像（→4.4 分钟）
> 两个手段叠加才真正解决问题 —— 只做其中一个都不够。

> 这一轮也留了个明确的下一步（见 6.2 第 4 条的相邻项）：`ffprobe` 单独占 53 MB，
> 而它和 `ffmpeg` 是同一套静态库编译出的两个前端。若把探测改成解析
> `ffmpeg -i` 的输出，就能再省掉整个 ffprobe —— 但那是一次有回归风险的探测层重构，
> 当前 77 项 smoke + 28 项真实素材用例是安全网，值不值得做取决于后续是否还要压体积。
...
| `release/Lumen-conv-便携版/` | **本轮首次产出的可分发产物**（Windows x64 便携版，602.4 MB，**不入库**，由 `npm run dist:portable` 生成） |

### 功能

- 需求 1：媒体信息探测 + 智能选帧缩略图（含磁盘缓存、后台预热）。
- 需求 2：7 种容器 / 9 个预设的 ffmpeg 命令行转换，命令行原文可在界面上查看。
- 需求 3：**12 项首轮自主优化**（硬件编码器真实试跑探测、智能选帧、杀进程树、自动改名、
  错误翻译、HDR 色调映射与旋转转正、只缩不放、GIF 调色板、直通预检、faststart、
  快速探测+后台完整探测、自定义协议）**+ 后续 8 轮迭代新增的能力**：
  - 三层决策界面（**用途 → 播放设备 → 参数**）+ 兼容性预检（把"转完才发现播不了"提前拦住）
  - 两种模式：推荐（选用途就够了）/ 自定义（纯参数面板，专业参数常开）
  - **实际输出尺寸**显示（修"界面显示了一个它不会产出的分辨率"）
  - **竖屏 9:16 适配**（补黑边 / 裁剪填满，画布短边取 min(上限, 源短边) 所以同样不放大）
  - **音频处理**：响度归一化（EBU R128）、音量增益、声道转换
  - **字幕烧录**（hardcode）+ 两道 block 级拦截
  - **队列控制**：暂停/继续、重排、并发、总剩余、磁盘空间预检
  - **Shell 集成**：命令行接文件 / 发送到 / 右键菜单（可撤销）
  - **单帧实时预览**（与原图并排、可放大到整屏）
  - 逐条对照见 `README.md` 的"功能特性"与"常用操作"两节。
- 需求 6：Electron 真桌面应用（自绘标题栏、原生对话框、资源管理器集成、系统通知、单实例、安装包目标）。
- 需求 7：拖拽即添加、空状态引导、失败任务不死路、深浅色主题、全中文文案（含错误翻译）、
  设置页诊断区、实时预计体积、快捷键；**并且每一处"用户会卡住"的地方都被真实反馈修过一遍**：
  模式的差异要落在首屏、自定义模式不要推荐、一个参数只能有一个主人、界面显示的尺寸必须是真实产物尺寸、
  预览要看得清、缺 ffmpeg 时必须说清原因（而不是让界面停在"检测中…"）。
  启发式取舍也写进了代码注释与文档：**警告要准但不替用户做决定**（磁盘空间预检只提示不硬拦；
  偏离推荐值只列出差异不强制回退）。

### 文档

- `README.md`：项目主文档（简介、**直接下载入口**、10 张界面截图、需求逐条对照、技术栈、ASCII 架构图、
  快速开始、目录结构、二进制获取与替换、常用操作（含 Shell 集成 / 音频字幕 / 队列 / 预览 / 精简版）、
  已知限制与后续计划、环境踩坑备忘、License）。
- `docs/SESSION_SUMMARY.md`：本文件。
- `docs/DECISIONS.md`：**28 条** ADR 风格决策记录（D-001 … D-028）。每条都写清"背景 / 决策 / 代价权衡 /
  验证方式"，并对被后续决策取代的条目加了指向标注（例如 D-018 的第 3、4 条被 D-019 取代）。
  D-017 打包方式、D-018~D-021 界面与参数归属、D-022 音频字幕、D-023 队列、D-024 Shell 集成、D-025 预览与精简版。
...
- `docs/screenshots/README.md`：截图清单（**10 张**，逐张给出断言依据与实测值）。界面截图由
  `npm run smoke:ui:file` → `npm run smoke:ui:full` 两步自动生成（见第 4.1 节）。
...
npm run smoke:ui:file  # 上一项 + 通过 __lumenAddFiles 加载真实视频后再截图（68 项检查）
npm run smoke:ui:full  # 再额外在应用内真的点一次「开始转换」并等任务跑完（84 项检查）
npm run smoke:ui:cli   # 命令行直接带一个视频路径启动，验证 Shell 集成入口（17 项检查）
npm run test:real:convert  # 真实素材端到端：探测 + 转换 + 产物回读（28 项）
npm run dist:portable:slim # 组装精简版（不内置 ffmpeg），并用它跑一次打包态自检
```
...
| **桌面应用已真正启动** | `npm run smoke:ui:full` 启动真实 Electron 窗口、加载真实视频、逐页截图、**在应用内真的点一次「开始转换」并等任务跑到终态**，**85/85 通过、退出码 0**，产出 9 张 PNG（见 4.1） |
| **便携版打包已产出并实机验证** | `npm run dist:portable`（`node scripts/package-portable.mjs --build`）**完全离线**产出 `release/Lumen-conv-便携版/`：`Lumen-conv.exe` **200.4 MB**（210,149,888 字节）、目录总计 **602.4 MB**；`release/` 已被 `.gitignore` 排除（`git check-ignore -v release/` 命中第 4 行）。产物**实跑全套界面自检 84/84 通过、退出码 0**（含应用内真实转换：状态 `done`、产物 3.15 MB、进度 100%），详见 4.1.1 |

#### 4.1.1 便携版（打包态）自检：`release/Lumen-conv-便携版/Lumen-conv.exe` **84/84 通过、退出码 0**
...
| 项目 | 实测结果 |
| --- | --- |
| 产物 | `release/Lumen-conv-便携版/Lumen-conv.exe` **200.4 MB**（210,149,888 字节）；目录总计 **602.4 MB** |
...
| 自检结果 | **85/85 通过、退出码 0**，其中包含应用内真实转换：任务状态 `done`、产物 **3.15 MB**、进度 `percent=100` |
...
#### 界面自检：三级命令，最高一级 `npm run smoke:ui:full` **85/85 全部通过**
...
三个级别是递进关系，检查项只在上一级基础上**追加**（14 → 69 → 84）：

| 命令 | 追加的动作 | 检查项 |
| --- | --- | --- |
| `npm run smoke:ui` | 启动窗口 → 等 `data-store-ready` → 逐页切换截图 | **14 项** |
| `npm run smoke:ui:file` | 加 `--smoke-file=…`，加载真实视频后再断言信息面板与缩略图 | **69 项** |
| `npm run smoke:ui:full` | 加 `--smoke-convert`，在应用内真的点一次「开始转换」并等任务跑到终态 | **84 项** |

`smoke:ui:file` 一级（69 项）的断言与实测结果：
...
**结果：85/85 通过，退出码 0，且控制台不再出现任何 `An object could not be cloned.` 错误**
...
| ~~字幕选择 / 多音轨映射~~ | **已补齐**：`test:real:convert` 用 `subs-multi.mkv`（2 条字幕轨）与 `multi-audio.mkv`（5.1 AC3 + 立体声 AAC）真实跑通，并新增**字幕烧录**用例（产物不再带字幕流、视频仍在） |
| ~~GitHub 推送~~ | **已完成**：代码已推送，并在 `v1.0.0` 发布 Release（含便携版附件，已验证匿名可下载）。见 4.4 |
| **Shell 集成的"真实写入注册表"** | 自检**有意不自动执行**（它会改用户注册表）。开发态的"拒绝写入"有断言覆盖；真正的写入 / 「发送到」/ 右键菜单效果需要**人工点一次**（步骤见 `docs/TEST_CASES.md` B 部分） |
| **精简版首次配置流程** | 精简版自检只验证到"启动后如实提示 ffmpeg 未就绪 + 给出引导"（15/15 通过）。"用户按引导指定路径后转换恢复正常"这条**端到端路径没有自动化覆盖** |
| **响度归一化的听感** | 验证到"滤镜进了命令、真实转码后声道数/时长正确"，但"音量确实被拉齐了、没有明显失真"属于**听感判断**，没有做客观测量（如转后 LUFS 回读） |
| **竖屏裁剪的画面损失** | 4K + 720p + 裁剪的产物尺寸已用 ffprobe 回读验证（720×1280），但"裁掉了哪些内容、是否可接受"需要人眼看 —— UI 里写了"约保留原宽的 X%"作为提示 |

> 本轮状态变化：
> ① 原先列在本表的 **`--smoke-convert`（应用内真跑转换）已移出**——`npm run smoke:ui:full` 实测 **85/85 通过、退出码 0**（含"按钮点击真的创建了任务"这条严格断言），证据见 4.1 与 `docs/screenshots/queue-done.png`。
...
产物 `release/Lumen-conv-便携版/` 已实机验证 **84/84 通过、退出码 0**（见 4.1.1）。
...
> **本节此前版本写的是"代码尚未提交、也尚未推送"，那是当时的事实。现在已经全部完成，
> 下面按实际状态重写 —— 保留这段说明是为了避免"文档说的和仓库不一致"这种最误导人的情况。**

仓库：<https://github.com/xiaomingliang927/Lumen-conv>
...
**推送过程中处理的两个环境问题**（都不是设计取舍，如实记录）：

1. **SSH 22 端口不通**：本机到 `github.com:22` 连接超时。改用 GitHub 的
   `ssh.github.com:443` 端点后认证成功；后来该端点也开始超时，最终把推送地址切到 **HTTPS**
   （凭据走 Git Credential Manager 已存的 token）。拉取/推送现在都走 HTTPS。
2. **中文附件名被 GitHub 清洗**：上传 `Lumen-conv-便携版-v1.0.0.zip` 后，附件名变成了
   `Lumen-conv-.-v1.0.0.zip` —— 别人下载时看着像坏文件。已用 API 的 `PATCH` 改名成
   `Lumen-conv-portable-v1.0.0.zip`（秒级，不必重传 237 MB），Release 正文里的文件名也同步了。

**二进制是否入库这个问题已经拍板**：`.gitignore` 排除 `resources/bin/`（ffmpeg 139.1 MB + ffprobe 138.9 MB）
与 `.downloads/`（归档缓存），取向是"仓库只放源码，二进制按需获取"。
...
| 13 | **「推荐 / 自定义」两种模式的差别全在折叠线以下，用户看不出区别**（本轮用户真实反馈，原话"推荐设置和自定义没区别啊"） | **功能存在 ≠ 用户感知到**：代码里两种模式确实不同（专业参数区块 + 帧率下拉只在自定义模式出现），但这两处都在 900px 之外。量化根因：① 「视频信息」默认展开占约 270px，把「用途」推到 376px；② 专业参数挂在面板末尾。修复四处：① 视频信息默认收起成一行摘要（`infoOpen`）；② 模式切换下方新增「当前方案摘要」`plan-summary`；③ 专业参数区块**整体搬到面板最上面**（仅自定义模式出现，改名「专业参数」）；④ **取消**"切到自定义自动展开专业参数" | 自检新增断言"自定义模式：专业参数入口出现在首屏"实测 `专业参数@152`；"两种模式在首屏就有可见差别"现在**把两种模式的首屏都打印出来**（推荐 `视频信息@152 / 你要拿去干什么@235 / 在哪播 / 多大体积@646`；自定义 `专业参数@152 / 视频信息@205 / 你要拿去干什么@288 / 在哪播 / 多大体积@699`）。`smoke:ui:file` **69/69**、`smoke:ui:full` **85/85**，退出码 0。对照截图 `mode-recommended.png` / `mode-custom.png`。决策与代价见 `DECISIONS.md` D-018 |
| 14 | **打包态有 8 项检查被静默跳过 —— 文档却宣称"与开发态完全相同"**（本轮核对覆盖范围时发现，是 ENOTDIR 那条的漏网之鱼） | 第 9 条修 `app.getAppPath()` 时只改了**截图目录**与 `--smoke-file=` 两处，**漏了第三处**：加载第二个样本的路径仍写 `path.join(app.getAppPath(), 'test-assets', …)`。打包态 `app.getAppPath()` 是 `...\resources\app.asar`（文件），拼出来的路径永远不存在 → `if (existsSync(secondSample))` 为假 → **跨文件保留 / 勾选 3 项 / 质量联动 4 项，共 8 项检查一次都没跑过**，而摘要照样打印"58/58 通过"。这就是"少跑"伪装成"通过"：**只要没人数检查项个数，就永远发现不了**。修复：① 路径改用 `smokeBaseDir()`；② 新增 `--smoke-assets=<目录>` 参数（打包态 exe 旁边没有 test-assets，测试素材也不该塞进分发包）；③ **样本缺失时不再静默跳过，而是明确 push 一条失败**，提示怎么修 | 实测：便携版 `--smoke --smoke-file=… --smoke-assets=<仓库 test-assets> --smoke-convert` → **84/84 通过、退出码 0**（修复前是 58/58 且无人察觉）；故意传一个不存在的 `--smoke-assets` 时 → **46/47、退出码 1**，失败项文字直接说明"这 8 项检查会被跳过"（这条是"失败必须可见"的反向验证）。另外顺带确认：GUI 子系统 exe 用 PowerShell 的 `& exe … \| Select-String` 拿到的**不是**进程真实退出码，要用 `Start-Process -Wait -PassThru` 读 `.ExitCode` |
| 15 | **自定义模式里还摆着一排"用途推荐卡片"，用户认为不专业**（本轮用户真实反馈，原话"自定义不要这个推荐，缺少专业性，这个用最初那个版本自己调整更合适"） | D-018 只解决了"两种模式看不出区别"，但用户的下一层意思是**自定义模式根本不该出现推荐**。修复：① 「你要拿去干什么」整块在自定义模式下不渲染（连同"已偏离推荐值"提示与用途小贴士——它们讲的全是推荐值）；② 专业参数**常开**，删掉折叠开关与 `advancedOpen` 状态；③ 摘要行在自定义模式下显示「自定义参数」，而不是用户看不到、也改不了的用途名；④ **兼容性预检拆成独立区块、两种模式都显示**（它讲的是事实不是推荐，而且自定义模式下更容易手选 H.265 配老电视）；⑤ 区块顺序改由 CSS `order` 控制（不复制模板）：专业参数 → 质量与尺寸 → 在哪播 / 多大体积 → 视频信息 → 兼容性预检 | 新增两条专门盯这次需求的断言：「自定义模式：不出现用途推荐卡片」实测卡片数 **0**；「推荐模式：用途推荐卡片齐全」实测 **8**；并把"自定义模式出现专业参数"升级为"**且字段直接可见，不需要再点一次**"。`smoke:ui:file` **69/69**、`smoke:ui:full` **85/85**，退出码 0；便携版 **84/84**、退出码 0。对照截图 `mode-recommended.png`（8 张卡片）与 `mode-custom.png`（无卡片、参数铺满首屏）。决策与代价见 `DECISIONS.md` D-019 |
| 16 | **「用途」与「在哪播 / 多大体积」耦合**（本轮用户真实反馈）—— 查下来是"语义重叠但没接上" | 三个具体问题：① 「目标体积上限」控件住在「在哪播」里、值却由用途决定（发微信=100MB），**一个参数两个主人**；② 用途有「老电视 / 车载 U 盘」，设备下拉有「老安卓电视 / 车机」，两处都在问设备；③ **选前者根本不会改后者**（`applyUseCase` 与 `chooseUseCase` 都不碰 `deviceId`），于是那个用途最该做的兼容性预检按"不限定"跑，等于没检查。修复：① `UseCase` 新增 `deviceId`，两条写入路径都补上（8 个用途各自映射）；② 设备提示写明"由用途「X」自动设定"，手动改过后改口为"已手动改过（推荐：Y）"并出现「用回用途推荐」；③ 体积上限抽成纯展示组件 `SizeLimitField.vue`，归还用途区（推荐模式）/ 质量区（自定义模式）；④ 区块改名「在哪播」，副标题"只用于兼容性预检" | 新增 3 条断言全部通过：「体积上限控件归属用途区」用途区内=true / 设备区内=false；「选老电视会带出对应设备」→ `android-old`；「设备提示写明来源且手动改过有恢复入口」。`smoke:ui:file` **69/69**、`smoke:ui:full` **85/85**，便携版 **84/84**，退出码均 0。决策与代价见 `DECISIONS.md` D-020 |
| 17 | **"换成手机"但屏幕比例没变 —— 界面显示了一个它不会产出的分辨率**（本轮用户真实反馈） | 根因：`分辨率` 是**上限**，命令构建有"只缩不放"规则（`smoke-test.mjs` 本来就有 `源分辨率低于目标时不放大` 这条用例）。640×360 的源选 1080p：界面写「1080p 全高清」、卡片写"按手机屏幕尺寸"，**产物还是 640×360**。修复：① 抽出 `shared/output-size.ts` 的 `planOutputSize()` 作为**输出尺寸与 ffmpeg 滤镜的唯一计算处**（`commands.ts` 用它返回的 `filters`，界面用它返回的 `width/height/note`）——显示与执行共用一份计划，就不可能再对不上；② 「质量与尺寸」新增「输出尺寸 640×360 · 源低于 1080p 上限，不放大」，分辨率标签补「（上限）」；③ 去掉「手机存着看」的过度承诺文案；④ 新增**默认关闭**的「画面比例」：竖屏 9:16 补黑边 / 裁剪填满，画布短边取 `min(上限, 源短边)`，竖屏下同样不放大 | `smoke` **77/77**（+4 条画面比例用例）、`test:real:convert` **28/28**（+3 条 ffprobe 回读：4K→**720×1280** 补边/裁剪、360p 源→**360×640** 不放大）、`smoke:ui:file` **69/69**（+3 条：输出尺寸显示 640×360 且标注不放大 / 切竖屏变 360×640 / 切回可还原）。新增截图 `docs/screenshots/size-plan.png` 作为视觉证据。<br>**过程中还抓到一个真 bug**：竖屏补边的 `force_original_aspect_ratio` 会算出小数尺寸（360×202.5），ffmpeg 取整后改 SAR 补偿，ffprobe 回读的 `displayWidth` 变成 **361** —— 真实素材测试抓到，补 `setsar=1` 后精确相符。见 `DECISIONS.md` D-021 |
| 18 | **音频处理缺失**（用户从"还能优化什么"清单里选的第 1 项）：此前只有"移除音频"（`-an`），没有任何音频能力 | 新增三项，全在「专业参数」里（自定义模式）：**响度归一化**（EBU R128 单遍 loudnorm，-16 LUFS / TP -1.5 / LRA 11）、**音量增益**（-30~+30 dB，越界夹紧）、**声道**（保持/单声道/立体声，显式选择优先于原有的自动下混推断）。**顺序是硬规则：先增益、后归一化** —— 反过来的话 loudnorm 会把用户加的增益重新拉平。直通与"移除音频"下不生成 `-af` | `smoke` 新增 6 条（loudnorm 滤镜 / 顺序 / 夹紧 / `-ac 1` / source 模式不加 `-ac` / copy 不生成 `-af`）；`test:real:convert` 新增 1 条真实转码（响度归一化 + -3dB + 单声道 → 产物确为 1 声道、时长不变）；`smoke:ui:file` 新增 1 条（三项控件存在且改动跨文件保留）。见 `DECISIONS.md` D-022 |
| 19 | **字幕烧录缺失**（第 2 项）：只能"保留字幕轨"，而**能否显示取决于播放器**（很多电视手机默认不显示），对方看到的仍是无字幕画面 | 新增 hardcode 烧录。两件最容易错的事集中在 `shared/subtitle-burn.ts`：**路径转义**（反斜杠/冒号/单引号/整体引号四步，中文空格都要保住）与**全局流 index → 第几条字幕流的 si 换算**（搞混的表现是"烧出来是另一条字幕"，不报错）。滤镜排在缩放之后（否则字幕跟着画面一起缩放）。**两道 block 级拦截**（构建期 + 兼容性预检各一份，都给一键修复）：直通模式不能烧录、图形字幕（PGS/VobSub）不能烧录。界面把"保留字幕轨"与"烧进画面"分成两组并分别解释代价 | `smoke` 新增 9 条（转义 2 / si 换算 / 两类拦截 2 / 滤镜顺序 / 预检 block+info 3）；`test:real:convert` 新增 1 条**真实内嵌字幕素材**烧录（产物不再带字幕流且视频仍在）；`smoke:ui:file` 新增 2 条（单选组、预检联动）。见 `DECISIONS.md` D-022 |
| 20 | **队列只能取消/重试/删除**（第 3 项）：不能暂停、不能重排、并发数藏在设置页、没有总剩余、盘满了要等 ffmpeg 转很久才报错 | 引擎加 `pauseQueue/resumeQueue/moveJob/queuedIds`。**暂停的语义定为"不再启动新任务，跑着的跑完"** —— ffmpeg 不支持断点续传，硬停一个跑一半的任务只能从头再来，那叫取消。重排只动排队数组，界面上重排按钮只在 `queued` 状态渲染。队列页可直接调并发（1~4）。总剩余 = 运行中 ETA + 排队数 × 已完成平均耗时，**没有历史数据时显示"—"不编数字**。开转前做**磁盘空间预检**（新 IPC `system:free-space`）：留 10% 余量、只说"可能不够"、不硬拦 | `smoke` 新增 2 条（暂停期间 0 个在跑 / 3 个停在排队，继续后确实启动；置顶/上移/下移/非法 id）；`smoke:ui:full` 新增 3 条走真实界面链路（点暂停 → 点开始转换 → 任务停在排队中且带 3 个重排按钮 → 点继续恢复）。**第一版给暂停按钮加了 `disabled`（队列空时禁用），把"先暂停再开转"这条常见路径堵死了 —— 是自检把它暴露出来的**。见 `DECISIONS.md` D-023 |
| 21 | **没有 Shell 集成**（第 4 项）：只能先开窗口再拖文件，离"桌面客户端"差一步 | 命令行接文件（`videoPathsFromArgv`：判据是"文件存在 + 扩展名在支持列表"，**不是**"取最后一个参数"——否则会把 `--smoke` 这类开关当成文件）；应用已开着时由 `second-instance` 把路径转交给现有窗口；走与拖拽**完全相同**的 `addFiles` 路径。「发送到」+ 右键菜单**只写 HKCU、可一键撤销、不动文件关联**（不抢用户原有播放器）；**开发态拒绝写入**（那时 exe 是 electron.exe，注册了只会启动裸 Electron）。多文件与纯音频场景一并支持 | 新增 `npm run smoke:ui:cli`（17 项）：命令行传入的文件必须自动出现在列表里 + 开发态必须拒绝写注册表（**安全性断言**）。**自检抓到一个真 bug**：主进程 `handle()` 返回 `{ok,data}`，而 preload 写成 `invoke(...) as Promise<string[]>`，`as` 把类型不匹配藏住了，运行时 `pending.length` 是 undefined、文件永远加载不出来。另修「空状态引导已显示」在带文件启动时的假失败（反向断言）。见 `DECISIONS.md` D-024 |
| 22 | **看不到"改了参数会变成什么样"**（用户勾选的第 5 项）：改分辨率/画面比例/裁剪后只能靠"预计体积"这一个间接信号猜 | 新增 `electron/ffmpeg/preview.ts`：把当前参数套用在**同一帧**上（抽帧时间点就用缩略图那个），左「原图」右「效果」并排。**滤镜链复用同一批规则**（尺寸来自 `planOutputSize`、字幕来自 `subtitle-burn`）—— 各写一份的话预览就会和产物对不上，那比没有预览更糟。防抖 400ms、只依赖影响构图的参数（调音量不重渲染）、用 token 丢弃过期结果。**能力边界写进界面文案**：能反映尺寸/比例/补边/字幕，**不能反映编码质量**（预览是无损 PNG），所以叫「画面效果预览」不叫「输出效果预览」 | `smoke:ui:file` 新增 2 条（原图与效果图并排且效果图来自 ffmpeg / 改画面比例后预览**会重新渲染**）。踩坑：`cacheDir()` 只拼路径不建目录，ffmpeg 报"打不开输出文件"，实际是目录不存在 —— 一轮自检留下 0 张图才发现。顺带加了过期预览图清理（保留 10 分钟）并纳入「清理缓存」。见 `DECISIONS.md` D-025 |
| 23 | **便携版 602 MB 太重**（第 6 项）：为了试一下要下 600 MB，其中 278 MB 是两个 ffmpeg 二进制 | 新增 `npm run dist:portable:slim`：不内置 ffmpeg，目录 **602.4 MB → 324.4 MB**，输出到**另一个目录**（不覆盖完整版，否则会让人以为瘦身没生效）。前提是应用本来就有"找不到 ffmpeg 的引导"，所以不是砍功能，而是把"自带"换成"你自己那份也能用"<br>**精简版逼出一个真 bug（本次最有价值的收获）**：`capabilities:get` 在找不到 ffmpeg 时是**抛错**的，于是渲染进程拿不到 capabilities，而引导横幅的显示条件 `capabilities !== null && !capabilities.ready` **永远不成立** —— 用户打开精简版转换全不可用，界面却没有任何解释，标题栏永远停在"检测中…"。这正踩在需求 7（"给人用"）最不能接受的形态上：**功能不可用且不说原因**。改为返回"未就绪"的能力对象 + 三条人话诊断 | `smoke:ui:file` 69/69、`smoke:ui:full` 85/85（开发态）、完整便携版 **84/84**、精简版自检 **15/15**。修复后精简版标题栏显示「ffmpeg 未就绪」、顶部横幅写明两步操作并给「去设置」按钮（截图见 `release/Lumen-conv-精简版/docs/screenshots/main.png`）。**打包态 83 项 / 开发态 84 项，差异逐条可查**（只在开发态跑 2 条：拒绝写注册表、拒绝创建快捷方式；只在打包态跑 1 条：包内有 ico），已写进 `DECISIONS.md` D-025 而不是含糊说"完全相同" |
| 24 | **预览太小看不清**（用户看到成品后的反馈）：第一版把预览放在右侧 400px 面板里，两张图各约 190px |  搬到**中间栏下半部分**（加载少量文件时那里本来是一大片空白）：两张图各约 390px，约 4 倍面积；新增**点击放大到整屏**（原图/效果可切换，Esc 关闭）；渲染宽度 480 → 960；文件列表改成不参与拉伸（`flex: 0 0 auto` + 最高 46%）。<br>**这条的教训**：从断言角度看第一版完全合格（两张图都在、参数一变就重渲染），但 190px 的格子里用户根本看不出差别 —— **断言测不出"够不够大"**，这类问题只能靠人看 |  自检同步更新选择器（`.preview-pane .preview-grid`）后 `smoke:ui:file` 69/69；截图 `main-with-file.png` 现在能直接看到并排的大图 |
| 25 | **桌面快捷方式与打包产物"不太一样"**（用户看到成品后的反馈，原话"这个快捷方式和直接打包不太一样吧"） |  查证发现用户说得对，而且是**三个问题叠在一起**：① 便携版包里**根本没有 `.ico`** —— 下载包的人无论怎么建快捷方式都只能拿到 exe 的内嵌图标；② 脚本建的快捷方式指向**仓库里的** `build/icon.ico`，那是仓库文件、不随分发包走；③ **`BrowserWindow` 从来没设过 `icon`** —— 所以不只桌面图标，**窗口左上角与任务栏**也是 Electron 默认的原子图标。<br>根因仍是那条老账：**exe 内嵌图标换不掉**（rcedit 在含中文的路径下失效，D-017），而"复制到 ASCII 临时路径再改回来"也走不通（用户名本身含中文）。<br>修法：把图标复制成**包内的 `Lumen-conv.ico`**，窗口图标、快捷方式、右键菜单三处都用它；快捷方式补上 `"%1"`（拖视频到快捷方式即可加载）；设置页新增「创建桌面快捷方式」按钮 —— 发布包是**一个 zip、里面没有快捷方式**，用户只能自己右键创建，而那恰好是拿不到我们图标的路径 |  新增 2 条断言：打包态「包内有 Lumen-conv.ico」、开发态「拒绝创建桌面快捷方式（避免指向 electron.exe）」；另用**测试名字**（不碰用户已有快捷方式）真实创建并回读 `.lnk` 属性：目标/图标/起始位置/参数/备注 **5/5 通过**，验证后立即删除。<br>全套回归：`smoke` 77/77、`test:real:convert` 28/28、`smoke:ui:file` **69/69**、`smoke:ui:full` **85/85**、`smoke:ui:cli` **17/17**、打包态 **84/84**。<br>**教训（两条）**：① "打包脚本里做了一件事"不等于"产物里有它" —— 第一版把复制图标写在 `createDesktopShortcut()` 里，而那个函数只在 `--shortcut` 时执行；② 在 PowerShell stdout 上比对中文路径会得到**假失败**（控制台编码破坏中文），改成"写 UTF-8 文件再读"才可靠。见 `DECISIONS.md` D-027 |
| 26 | **「原图 / 效果」并排预览其实是两帧**（本轮自查截图时发现：左侧水印 `LUMEN 0s`、右侧 `LUMEN 1s`） 并排放在一起的语义就是"**同一帧、参数不同**"，但这条保证实际上要**三层都修对**才成立 —— 而且每一层都能让断言全绿： <br>**第一层（用错了时间点）**：缩略图的真实抽帧点从没被产品代码消费。`probe.ts` 另算一份 `thumbnailAtSec`（时长 10%、下限 1 秒），而 `thumbnail.ts` 的智能选帧**自己**按 10%/25%/50%/5% 算候选点并避黑场，两者经常不是同一时刻，`PreviewPane` 用的却是前者 —— 左边第 0 秒、右边第 1 秒。 <br>**第二层（接上了也不会重渲染）**：把真实帧时间接上后，标题写着「同取 00:00」，**图还是 1 秒那张**。因为探测先完成、缩略图后到，预览在缩略图回来之前就渲过一次（那时只能用 probe 的时间点），而帧时间**不在 `previewKey` 里**，真实帧时间到达时并不触发重渲染：文字更新了，图没更新。 <br>**第三层（0 被当成"没指定"）**：补上重渲染后仍然不对。真凶在 IPC handler 一行里：`const at = atSec > 0 ? atSec : probe.thumbnailAtSec || …` —— **第 0 帧是合法取值**，而 `> 0` 把它当成"没传"，于是"请给我第 0 帧"被悄悄换成 `probe.thumbnailAtSec`（6 秒样本上正是 1 秒）。这是本项目第二次踩"0 被当成空值"（另一次是 `formatDuration(0)` 在界面上显示成 `—`）。 <br>**修法**：① `getThumbnail()` 返回的 `atSec` 落到 `LoadedFile.thumbnailAtSec`，预览改用它；② 命中缓存时它原来返回 `atSec: -1`（"不知道是哪一帧"），所以把真实帧时间持久化成缓存旁车文件 `<key>.json`；③ 帧时间加进 `previewKey`；④ 标签只描述**已经显示出来**的那一帧（在 `<img>` 的 `load` 事件里推进，而不是 URL 一赋值就改）；⑤ handler 判存在改用 `>= 0`；⑥ 缩略图帧与预览帧分别挂到 `data-thumb-at` / `data-preview-at`，自检断言**两者必须相等且非空**。 断言从"两张图都在"升级为**同帧的机器可判定形式**：`data-thumb-at`（缩略图取自第几秒）必须等于 `data-preview-at`（预览实际渲染第几秒），截图前还要等这两个值相等 **并且不在渲染中**才拍。`smoke:ui:file` **69/69**、`smoke:ui:full` **85/85**、`smoke` **77/77**（缓存命中那条仍断言 `cached=true, 1ms`）。最终用**抠图哈希**验证：把渲染出的效果图与 `-ss 0.000` / `-ss 1.000` 两张参考帧的左上角水印区域分别哈希，效果图与 **0 秒那张完全相同**（`35E01F47…`），与 1 秒那张不同。<br>**教训（三条）**：① 断言只能证明"有图"，证明不了"这两张图可比"；② 属性/DOM/日志可以对，而**像素是错的**——这一层只能靠把图抠出来比对；③ 帧时间这种东西**0 是合法值**，"没传"与"传了 0"必须用 `>= 0` / `undefined` 区分，不能用真假值
| 27  |  **"这个地址用不了"**（用户打开 Release 准备下载时的反馈，原话"帮我自己做成超链接放在下载的前面，实测这个 https://ghfast.top/… 可以用"） |  反馈里是两件事：① **镜像地址写在代码块里，点不了** —— 只能复制再粘到地址栏；而这段代码块恰恰就是"下载慢怎么办"的解法，**最该一步点到的地方反而最难用**；② **直连在他那里"用不了"**，而文档一直把直连当作"慢但能用"的兜底。修法：README 与 Release 说明的下载入口都改成**可点的镜像超链接并放在最前面**（推荐附件用标题级链接），表格拆成「加速下载（推荐）/ 直连 GitHub」两列，地址构造规则与备用镜像（`gh-proxy.com` / `ghproxy.net`）放到表后；顺手修掉之前把说明插进表格中间、导致**下载表被切成两半（表头丢失）**的问题 |  **同时纠正了一个自己写错的判断**：复查时直连请求**直接失败**，而四个镜像（`ghfast.top` / `gh-proxy.com` / `ghproxy.net` / `hk.gh-proxy.com`）全部返回 HTTP 206、文件头魔数与大小都正确 —— 所以不是"直连慢"，而是**"直连可能根本连不上"**，兜底方向也就反了（默认给镜像、直连作备选）。<br>验证：用脚本对 `README.md` 断言"镜像链接出现在下载表之前""表头含两列""镜像链接 ≥ 4 条"（4 条），并核对 Release 说明 97 行结构完整（`<details>` 开闭配对、四节内容齐全）。<br>**教训**：把"怎么绕过困难"的方法放进代码块，等于把它变成**需要用户手工执行的操作**；给人用的文档里，**能点的链接 > 可复制的文本**
| 28  |  **"这个预计不准"**（本轮用户真实反馈，配图是底部「预计 100 MB · 均衡（推荐）」那一行） |  源是 6 秒 / 661 KB 的小样本，产物不可能有 100 MB —— 用户一眼就看出来了。查下来不是算错，而是**语义搞错了**：旧实现在有"体积上限"时直接返回 `{ bytes: 上限 }`，还标成 `approximate: false`（"两遍编码精确命中"）。于是"发微信 / QQ（上限 100 MB）"这条**最常用**的路径上，不管源是 6 秒彩条还是 2 小时 4K，界面永远显示「预计 100 MB」，而真实产物 3.16 MB —— **差 31.6 倍**。修法：① 预计值改为 `min(按内容的码率估算, 上限)`，界面上把"预计"与"上限"**分开写**（「预计 3.71 MB · 均衡（推荐） · 上限 100 MB（上限用不到）」）；② **有上限时不读"当前质量档"** —— 命令走 `-b:v <上限反推码率>`，质量档位根本不参与决定，读它是"估算依赖了一个不影响结果的输入"（这条是自检逼出来的：它会把质量改成「极小体积」去验 CRF 路径，跟着读就会算出 0.93 MB）；③ 一律标成估算；④ 界面文案改口径：上限是"不许超过"，不是目标值 |  新增 1 项断言把语义钉住：预计值挂在 `data-estimate-bytes`、产物实际大小挂在 `data-bytes`，**两者比值必须落在 1/3 ~ 3 倍之间**（3 倍是刻意宽松的门槛 —— 码率模型本来就是估算，但"差一个数量级"必须能被抓住）。<br>实测：修前 **31.6×**（100 MB / 3.16 MB），修后 **1.18×**（预计 3.71 MB / 实测 3.16 MB）。<br>全套回归：`smoke` 77/77、`test:real:convert` 28/28、`smoke:ui:file` 69/69、`smoke:ui:full` **85/85**、`smoke:ui:cli` 17/17、打包态 **84/84**。<br>**教训**：一个**上限**被当成**预计值**展示了很久，还标着"精确" —— 这类错误的特点是**界面语法完全正确、断言也全绿**（当时只断言"文本里含预计"），只有人看一眼数字量级才能发现。所以新断言的判据不是"有没有显示"，而是"**和真实产物对不对得上**"。见 `DECISIONS.md` D-028
...
> 而不是从旧源码里读出来的。修复后的代码、实验输出与 `smoke:ui:full` 的 85/85 都是可直接核对的。
...
| 10 | **`probe.ts` 的 `thumbnailAtSec` 与 `thumbnail.ts` 的候选点逻辑重叠** | `probe.ts` 用 `clampThumbnailTime()` 算出"时长的 10%、下限 1 秒、上限 30 秒"；`thumbnail.ts` 的 `getThumbnail()` 在 `smart` 模式下**自己按 10%/25%/50%/5% + 2 秒另算一份候选点**，并不读 `thumbnailAtSec`。两处逻辑仍然重叠，**但用户可见的后果已在第 26 条修掉**：`getThumbnail()` 返回的"实际抽帧点"现在会落到 `LoadedFile.thumbnailAtSec` 供画面预览取同一帧（`probe.thumbnailAtSec` 不再被产品代码消费，只剩 `scripts/smoke-test.mjs` 的一条区间断言在读它）。**该合并的仍然是**：`probe.ts` 那份 `clampThumbnailTime()` 已无消费者，应当删除或改为由 `thumbnail.ts` 回报 |
...
### 6.1 曾经列在这里、现已完成（移出待办，避免"做完的事还挂在待办上"）

| 原待办 | 现状 |
| --- | --- |
| ~~决定二进制是否入库，然后完成首次提交与推送~~ | **已完成**：`.gitignore` 排除二进制；代码已推送；`v1.0.0` Release 已发布（见 4.4） |
| ~~补带字幕与多音轨的样本~~ | **已完成**：`subs-multi.mkv`（2 条字幕）与 `multi-audio.mkv`（5.1 AC3 + 立体声 AAC）已纳入 `test:real:convert` |
| ~~补真正带旋转元数据的样本~~ | **已完成**：用 `-display_rotation 90` 造出 `rot90.mp4`，并借此发现"手动 transpose 会转两次互相抵消"的真 bug |
| ~~让界面上能看见已实现的全部快捷键~~ | **未做**，仍在下面的待办里（第 8 条） |

### 6.2 仍然待做（按优先级）

1. **在有网络的环境产出并验证 NSIS 安装包**（当前最大的空白）：便携版与精简版都已实测跑通，
   缺的是**安装包** —— `npm run dist:nsis` 需要额外工具链与独立的 Electron 缓存，本机受限网络下做不出来（见 4.3）。
   需要在联网环境执行，并在安装后确认 `resources/bin/{ffmpeg,ffprobe}.exe` 被正确释放
   （`process.resourcesPath/bin`）、能真的转一个文件、且 exe 的图标与版本信息被正确写入。
2. **人工走一遍 Shell 集成的"真实写入"**：自检有意不自动改用户注册表。
   需要手动点一次「设置 → 系统集成 → 一键开启」，然后验证右键菜单与「发送到」都能启动并加载文件，
   再点「移除」确认注册表项与快捷方式被清理干净（步骤见 `docs/TEST_CASES.md` B 部分）。
3. **补上仍未覆盖的样本**：HDR 片源（`zscale`/`tonemap` 链一次都没执行过）、
   **带片头黑场的视频**（用于证明 `blackdetect` + `signalstats` 真的能跳过黑帧 ——
   目前 `testsrc2` 素材没有黑场，只能证明链路跑得通）。
3b. **（可选）去掉 `ffprobe` 以再省 53 MB**：它与 `ffmpeg` 是同一套静态库编译出的两个前端，
   功能上无法互相替代，但媒体信息可以改成解析 `ffmpeg -i` 的输出。
   收益是发布包再小 53 MB；代价是探测层重构，涉及时长/码率/旋转矩阵/HDR 色彩信息等字段的重新解析，
   属于**有回归风险**的改动 —— 现有 77 项 smoke + 28 项真实素材用例能做安全网，但需要专门排一轮时间。
4. **引入单元测试框架**（Vitest）：`format.ts`、`progress.ts`、`sanitizeFileName()`、
   `planOutputSize()`、`checkCompatibility()` 这些纯函数值得边界穷举。
   本轮已经两次出现"命令拼装错误靠端到端才发现"（字幕滤镜顺序、竖屏 SAR 偏移），单测能把这类问题提前。
   顺带可以加 GitHub Actions（`windows-latest` 跑 `typecheck + smoke + smoke:ui:file`），让仓库页直接看到通过状态。
5. **响度归一化做客观验证**：目前验证到"滤镜进了命令、真实转码后声道与时长正确"。
   可以再用 ffmpeg 的 `ebur128` 统计回读产物的 LUFS，把"音量确实被拉齐"变成数字而不是感觉。
6. **把"预览"从只反映画面扩展到也能提示画质**：现在的预览是无损 PNG，答不了"压到 2 Mbps 会不会糊"。
   可选的下一步：用**目标码率编一小段**（例如 3 秒）再抽帧，代价是慢一些，但能真的看到压缩痕迹。
7. **清理未引用的预留代码**：`pickBestVideoCodec()`、`clearCapabilityCache()`、`warmThumbnail()`、`extractFrameAt()`
   —— 要么接进 UI（例如"手动挑封面"用 `extractFrameAt()`），要么删掉（第 5.2 节第 8 条）。
8. **合并重复逻辑**：`probe.ts` 的 `thumbnailAtSec` 与 `thumbnail.ts` 的候选点计算；
   `commands.ts` 的 `quoteArg()` 与 `convert.ts` 的 `formatCommand()` 两份命令行转义实现（第 5.2 节第 10 条）。
9. **让界面显示已实现的全部快捷键**（`Ctrl+1/2/3` 目前只能从代码注释里看到）。
10. **给 GIF 加硬性限制**：时长超过阈值时提示或自动截取前 N 秒，而不是只靠预设 `tip` 文字（第 5.2 节第 5 条）。
11. **评估接入硬件解码**（`-hwaccel` 系列参数），当前只用了硬件编码。
12. **可以考虑但明确未做的大件功能**：多视频合并/拼接、抽帧导出图片序列、变速、空间裁剪（框选区域）、
    自定义预设保存、文件夹监视自动转换、系统托盘常驻。
    这些都在 `docs/FEATURES_AND_ROADMAP.md` 里列着，**本轮没有实现，不要当成已支持**。
13. 其他工程项：任务队列持久化（重启后保留历史）、缩略图/预览缓存的容量上限与 LRU、
    打包产物的代码签名与自动更新（都需要证书或服务器）、
    以及给 `package-portable.mjs` 找一个不依赖具体环境的图标写入方式
