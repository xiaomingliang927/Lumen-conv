...
- **便携版**：`npm run dist:portable` 离线手工组装（`release/Lumen-conv-便携版/`，602.4 MB），
  实测自检 **83/83 通过、退出码 0**
- **精简版**：`npm run dist:portable:slim`（282.2 MB），实测 **14/14 通过**
...
| **精简版首次配置流程** | 精简版自检只验证到"启动后如实提示 ffmpeg 未就绪 + 给出引导"（14/14 通过）。"用户按引导指定路径后转换恢复正常"这条**端到端路径没有自动化覆盖** |
