...
npm run smoke:ui:file  # 上一项 + 通过 __lumenAddFiles 加载真实视频后再截图（62 项检查）
...
| 23 | **便携版 602 MB 太重**（第 6 项）：为了试一下要下 600 MB，其中 278 MB 是两个 ffmpeg 二进制 | 新增 `npm run dist:portable:slim`：不内置 ffmpeg，目录 **602.4 MB → 324.4 MB**，输出到**另一个目录**（不覆盖完整版，否则会让人以为瘦身没生效）。前提是应用本来就有"找不到 ffmpeg 的引导"，所以不是砍功能，而是把"自带"换成"你自己那份也能用"<br>**精简版逼出一个真 bug（本次最有价值的收获）**：`capabilities:get` 在找不到 ffmpeg 时是**抛错**的，于是渲染进程拿不到 capabilities，而引导横幅的显示条件 `capabilities !== null && !capabilities.ready` **永远不成立** —— 用户打开精简版转换全不可用，界面却没有任何解释，标题栏永远停在"检测中…"。这正踩在需求 7（"给人用"）最不能接受的形态上：**功能不可用且不说原因**。改为返回"未就绪"的能力对象 + 三条人话诊断 | `smoke:ui:file` 68/68、`smoke:ui:full` 83/83（开发态）、完整便携版 **82/82**、精简版自检 **14/14**。修复后精简版标题栏显示「ffmpeg 未就绪」、顶部横幅写明两步操作并给「去设置」按钮（截图见 `release/Lumen-conv-精简版/docs/screenshots/main.png`）。**打包态比开发态少 1 项是有意的**（那条注册表安全断言只在开发态成立），已写进 `DECISIONS.md` D-025 而不是含糊说"完全相同" |
